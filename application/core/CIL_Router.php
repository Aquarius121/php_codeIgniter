...
class CIL_Router extends CI_Router {
	...
	// do not adjust segments
	// as request is validated here
	function _use_validated_request()
	{
		return $this->use_validated_request;
	}
	...
	{
		return array_filter(explode(DIRECTORY_SEPARATOR, 
			build_path($this->base_suffix,
				$this->fetch_directory(),
				$this->fetch_class(true))));
	}
	 ...
	 * @return	string
	 */
	function fetch_class($without_suffix = false)
	{
		// attempt to load main 
		if (!($class = $this->class))
			$class = 'main';

		// don't append controller
		if ($without_suffix)
			return $class;
		
		// this doesn't exist when first called
		// to load the file based on class name
		if (class_exists("{$class}_Controller", false))

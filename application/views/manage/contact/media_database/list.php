...
<div class="table-responsive">
	<table class="grid table" id="selectable-results">
				...
				<td class="left">
					<div class="md-contact-hover-profile-container">
						...
						<img src="<?= Stored_File::url_from_filename($result->picture->finger) ?>"
							...
							class="contact-picture md-contact-picture md-profile-activator" />
						<?php else: ?>
					...
					<h3 class="contact-name nopadbot">
	...
	$(function() {
		...
		results.on("click", "tr", function(ev) {
			...
			if (target.hasClass("md-profile-activator")) return;
	...
	<div class="clearfix md-bottom-buttons listing-bottom-buttons">
		<?php if ($vd->results): ?>
		<div class="pull-left pad-15 col-lg-6" id="selectable-controls">
			...
			$(function() {
				...
				$("#contact-list-id").on_load_select({
			...
			<button type="button" name="add_to_list" id="md-add-to-list"
				class="btn btn-small btn-primary md-add-to-list">Add to List</button>
			<button type="button" id="md-create-list"
				class="btn btn-small btn-default">Create List</button>
		</div>
		<?php endif ?>
...
$(function() {
	...
	var add_to_list_url = "manage/contact/media_database/add_to_list";

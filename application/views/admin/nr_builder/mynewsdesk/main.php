<form method="post" id="selectable-form" action="admin/nr_builder/mynewsdesk/bulk_build" name="selectable_form">
				...
				<div class="row-fluid">
					...
					<div class="span12">
						<div class="pull-right">
							...
							<a href="admin/nr_builder/mynewsdesk/all?filter_search=CHECK_WEBSITE<?= 
								$vd->category_filter ?>">Check Website</a> |

							<a href="admin/nr_builder/mynewsdesk/all?filter_search=CHECK_LOGO<?= 
								$vd->category_filter ?>">Check Logo</a> |

							<a href="admin/nr_builder/mynewsdesk/all?filter_search=READY_TO_BUILD_NEWSROOMS_EN_NEW<?= 
								$vd->category_filter ?>">Ready NRs Eng (New)</a> | 

							<a href="admin/nr_builder/mynewsdesk/all?filter_search=READY_TO_BUILD_NEWSROOMS_EN_OLD<?= 
								$vd->category_filter ?>">Ready NRs Eng (Old)</a> | 

							<a href="admin/nr_builder/mynewsdesk/all?filter_search=READY_TO_BUILD_NEWSROOMS_NON_EN_NEW<?= 
								$vd->category_filter ?>">Ready NRs Non/Part. Eng (New)</a> | 

							<a href="admin/nr_builder/mynewsdesk/all?filter_search=READY_TO_BUILD_NEWSROOMS_NON_EN_OLD<?= 
					...
					<div class="span12">
						<div class="span6">
							<div class="pull-right pad-10v">
								<a href="admin/nr_builder/mynewsdesk/all?filter_search=ONLY_MISSING_LOGO_NEWSROOMS<?= 
									$vd->category_filter ?>">Logo Missing</a> | 

								<a href="admin/nr_builder/mynewsdesk/all?filter_search=ONLY_MISSING_SOCIALS_NEWSROOMS<?= 
									$vd->category_filter ?>">Socials Missing</a> | 

								<a href="admin/nr_builder/mynewsdesk/all?filter_search=ONLY_MISSING_EMAIL_NEWSROOMS<?= 
									$vd->category_filter ?>">Email Missing</a> | 

								<a href="admin/nr_builder/mynewsdesk/all?filter_search=DUPLICATE_EMAIL_NEWSROOMS<?= 
						...
						<div class="span6">
							<div class="span5">							
								<select name="country" id="country"
									...
									<option value="" class="status-false" selected>Filter by Country</option>
									<?php foreach ($vd->countries as $country): ?>
									<option value="<?= $country->name ?>"
										<?= value_if_test($country->name == @$vd->selected_country, 'selected')?>
										><?= $vd->esc($country->name) ?></option>
									<?php endforeach ?>
								</select>
							</div>
				...
				<table class="grid grid-tickboxes writing-orders-grid">
					<thead>
						<tr>
							...
							<th class="tb"></th>
							<th class="tb logo-status-th">Website Status</th>
						...
						<tr data-id="<?= $result->id ?>" class="result">
							...
							<td class="left">
								...
								<ul>
									...
									<li><a href="admin/nr_builder/mynewsdesk/delete/<?= $result->source_company_id?>"
										>Delete</a></li>
										<?php if (count($result->num_contacts)): ?>
											<li><a href="#" class="show-contacts" 
												data-id="<?= $result->source_company_id ?>">
												<?= $result->num_contacts ?> 
												Contact<?= value_if_test($result->num_contacts > 1, 's') ?>
											</a></li>
										<?php endif ?>
								</ul><br>
							...
							<td class="grid-social">
								<?php if (!empty($result->flag)): ?>
									<img src="<?= $result->flag ?>" alt="<?= $result->country ?>"
										title="<?= $result->country ?>" class="nomar pad-5">
								<?php endif ?>
							</td>

							<?= $this->load->view('admin/nr_builder/partials/nr_builder_tds/website_check', 
								array('result' => $result), false) ?>

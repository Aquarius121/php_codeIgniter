<form method="post" id="selectable-form" action="admin/newsroom_builder/">
<div class="row-fluid">
	<div class="span12">
...
<?= $this->load->view('admin/companies/newsroom_builder/sub_menu') ?>
...
<div class="list-filters">
	<div class="list-filter-header">filters active</div>
		...
		<div class="value">
			...
			<?php endif ?>
			<a class="remove" 
				href="admin/newsroom_builder/claim_submissions"></a>
		</div>
	</div>
</div>
<?php endif ?>
			...
			<div class="row-fluid">
				<div class="span12">
					<div class="pull-right">
						View: 
						<a href="admin/newsroom_builder/set_listing_chunk_size/20/<?= $vd->tab_filter ?><?= 
							$vd->esc(gstring()) ?>"
							<?= value_if_test($vd->listing_size == 20, " class='strong' ") ?>>20</a> | 
						<a href="admin/newsroom_builder/set_listing_chunk_size/50/<?= $vd->tab_filter ?><?= 
							$vd->esc(gstring()) ?>"
							<?= value_if_test($vd->listing_size == 50, " class='strong' ") ?>>50</a> | 
						<a href="admin/newsroom_builder/set_listing_chunk_size/100/<?= $vd->tab_filter ?><?= 
							$vd->esc(gstring()) ?>"
							<?= value_if_test($vd->listing_size == 100, " class='strong' ") ?>>100</a> | 

						<a href="admin/newsroom_builder/set_listing_chunk_size/475/<?= $vd->tab_filter ?><?= 
							$vd->esc(gstring()) ?>"

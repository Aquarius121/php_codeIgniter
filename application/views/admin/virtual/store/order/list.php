...
<?= $this->load->view('admin/virtual/store/partials/tabs') ?>
		...
		<div class="content listing">
			...
			<table class="grid">
				...
				<tbody class="results">
					...
					<tr data-id="<?= $vd->esc($result->id) ?>" class="result">
						
						<td class="left">
							<h3>
								<a href="common/vuras/<?= $result->user->id ?>/order/<?= $vd->esc($result->id) ?>" target="_blank">
									<strong><?= strtoupper(substr($result->id, 0, 8)) ?></strong><?= 
										strtoupper(substr($result->id, 8)) ?>
								</a>
							</h3>
							<ul>
								<li><a href="common/vuras/<?= $result->user->id ?>/order/<?= $vd->esc($result->id) ?>"

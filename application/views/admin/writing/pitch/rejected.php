				...
				<tbody class="results">
					<?php foreach ($vd->results as $result): ?>
						<tr data-id="<?= $result->id ?>" class="result">
							...
							<td>
								<?php if ($result->status == Model_Pitch_Order::STATUS_CUSTOMER_REJECTED): ?>
									<a href="admin/writing/pitch/review_single/<?= $result->order_id ?>">View Comments</a>
                                    <div>(Customer Rejected)</div>
								<?php else : ?>
                                	Sent to Writer	
                                <?php endif ?>
							</td>					

			...
			<div class="span8 information-panel">
				...
				<section class="form-section user-details">
					...
					<div class="row-fluid">
						...
						<div class="span6 relative">
							<?= $this->load->view('admin/store/coupon/partials/expiry-date') ?>
						</div>
					</div>
					...
					$(function() {
						...
						code.on("change", function() {
							...
							$.post("admin/store/coupon/code_check", data, function(res) {
							...
							<div class="span10">
								<select name="item_restriction[]" data-live-search="true"
									...
									<option value="<?= $item->id ?>"
										data-content="
											<strong class='status-false'><?= $item->id ?></strong> 
											&nbsp;<?= $vd->esc($item->name) ?> 
											<span>
												<?php if ($item->comment): ?>	
													<span class='status-black'>
														<?= $vd->esc($item->comment) ?><br>
													</span>
												<?php endif ?>
												Price: $<?= $item->price ?>
											</span>"
							...
							<div class="row-fluid item-specific-price-row">
								<div class="span5">
									<select name="item_list[]" 
										...
										<?php foreach ($vd->items as $item): ?>
										<option value="<?= $item->id ?>"
											data-content="<strong class='status-false'><?= $item->id ?></strong> 
												&nbsp;<?= $vd->esc($item->name) ?> 
												<span>
													<?php if ($item->comment): ?>	
														<span class='status-black'>

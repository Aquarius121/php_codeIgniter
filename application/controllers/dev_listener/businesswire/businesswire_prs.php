...
class BusinessWire_PRs_Controller extends Iella_Base {
	...
	public function save()
	{
		...
		{
			if ($m_businesswire_pr = Model_PB_BusinessWire_PR::find('dev_site_content_id', $pr_rec->id))
			{
				if (! $m_content = Model_Content::find($m_businesswire_pr->content_id))
					$m_content = new Model_Content();
				
				if (! $m_content_data = Model_Content_Data::find($m_businesswire_pr->content_id))
					$m_content_data = new Model_Content_Data();

				if (! $m_pb_scraped_c = Model_PB_Scraped_Content::find($m_businesswire_pr->content_id))
					$m_pb_scraped_c = new Model_PB_Scraped_Content();
			}
			else
			{
				$m_businesswire_pr = new Model_PB_BusinessWire_PR();
				$m_content = new Model_Content();
				$m_content_data = new Model_Content_Data();
				$m_pb_scraped_c = new Model_PB_Scraped_Content();
			}
			...
			$m_businesswire_pr->dev_site_content_id = $pr_rec->id;
			$m_businesswire_pr->content_id = $m_content->id;
			$m_businesswire_pr->url = $pr_rec->url;
			$m_businesswire_pr->cat_id = $pr_rec->cat_id;
			$m_businesswire_pr->web_video_provider = $pr_rec->web_video_provider;
			$m_businesswire_pr->web_video_id = $pr_rec->web_video_id;
			$m_businesswire_pr->cover_image_url = $pr_rec->cover_image_url;
			$m_businesswire_pr->businesswire_category_id = $pr_rec->businesswire_category_id;
			$m_businesswire_pr->businesswire_company_id = $pr_rec->businesswire_company_id;			
			$m_businesswire_pr->save();
			...
			if ($m_businesswire_cat = Model_BusinessWire_Category::find($pr_rec->businesswire_category_id))

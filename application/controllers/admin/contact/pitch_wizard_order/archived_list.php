...
class Archived_List_Controller extends Main_Controller {

	public function __construct()
	{
		parent::__construct();
		
		$this->vd->title[] = 'Archived Lists';
	}

	public function archive($pitch_list_id)
	{
		$pl = Model_Pitch_List::find($pitch_list_id);

		if ($pl)
		{
			$feedback = new Feedback('success');
			$feedback->set_title('Success!');
			$feedback->set_text('List Archived.');
			$this->add_feedback($feedback);
			$pl->is_archived = 1;
			$pl->save();
		}

		$this->redirect('admin/contact/pitch_wizard_order/all_list');
	}
	...
	{		
		...
		$url_format = gstring('admin/contact/pitch_wizard_order/archived_list/-chunk-');

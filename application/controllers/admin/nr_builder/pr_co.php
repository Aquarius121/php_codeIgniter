...
class PR_Co_Controller extends NR_Builder {

	public $title = 'Newsroom Builder | PR_Co';

	public function __construct()
	{
		parent::__construct();
		$this->vd->nr_source = Model_Company::SOURCE_PR_CO;
		$this->nr_source = Model_Company::SOURCE_PR_CO;
		$this->vd->nr_source_title = 'PR.Co';
	}
	...
	public function set_listing_chunk_size($size  = 0, $tab = null)
	{
		if ($size > 0)
		{
			...
			$url = gstring("admin/nr_builder/pr_co/{$tab}");
			$this->redirect($url);
		}
	}
	...
	{	
		$this->redirect("admin/nr_builder/pr_co/all");
	}

	// Pulling on demand all data from
	// PR_Co newsroom for a company	
	public function pull_nr_data($company_id)
	{
		if (empty($company_id))
			$this->redirect("admin/nr_builder/pr_co/all");

		if (!$mnd_company = Model_PR_Co_Company::find('company_id', $company_id))
			$this->redirect("admin/nr_builder/pr_co/all");
		...
		if (!$mnd_company->is_images_fetched)
			$this->pull_images($company_id);

		$this->vd->newsroom = Model_Newsroom::find($company_id);
		$mnd_company = Model_PR_Co_Company::find(array('company_id', $company_id));
		$this->vd->pr_co_company_id = $mnd_company->id;

		$this->load->view('admin/header');
		$this->load->view('admin/nr_builder/menu');
		$this->load->view('admin/pre-content');
		$this->load->view('admin/nr_builder/pr_co/pull_data_status');
	...
	{
		$command = "php-cli newswire.php cli auto_create_nr pr_co pull_contacts {$company_id}";
	...
	{
		$command = "php-cli newswire.php cli auto_create_nr pr_co pull_prs {$company_id}";
		$command = "nohup {$command} >/dev/null 2>&1 &";
		exec($command);
	}
	...
	{
		$command = "php-cli newswire.php cli auto_create_nr pr_co pull_images {$company_id}";
		$command = "nohup {$command} >/dev/null 2>&1 &";
		exec($command);
	}

	public function status_poll($pr_co_company_id)
	{
		$response = new stdClass();

		$mnd_c_data = Model_PR_Co_Company_Data::find($pr_co_company_id);
		...
		if ($mnd_c_data->is_prs_fetched == 1 && $mnd_c_data->is_contacts_fetched == 1
	...
	public function instant_edit_save()
	{
		...
		{
			$c_data = Model_PR_Co_Company_Data::find($company_id);
				...
				|| $field_name == "soc_gplus" || $field_name == "soc_youtube" || $field_name == "soc_pinterest"))
		{
			...
			$c_data->{$feed_status_field} = Model_PR_Co_Company_Data::SOCIAL_NOT_CHECKED;
			$c_data->save();
			$response = "Saved successfully";
		}
	...
	protected function fetch_auto_built_nr_results($chunkination, $filter = null, $pre_exist_nr_websites_sql = '')
	{
		...
		$l_join = "INNER JOIN (
						SELECT pr_co_company_id, SUM(ISNULL(language)) 
						FROM nr_pb_pr_co_content mc
						INNER JOIN nr_content c
						ON mc.content_id = c.id
						AND c.type = '{$pr}'
						GROUP BY pr_co_company_id 
						HAVING SUM(ISNULL(language)) = 0
					) AS l_counter ON l_counter.pr_co_company_id = cbc.id

					LEFT JOIN (
						SELECT pr_co_company_id, 
						COUNT(pr_co_company_id) AS num_en_prs
						FROM nr_pb_pr_co_content mc
						INNER JOIN nr_content c
						ON mc.content_id = c.id
						AND c.type = '{$pr}'
						WHERE language = 'en' 
						GROUP BY pr_co_company_id 
					) AS e_counter ON e_counter.pr_co_company_id = cbc.id
					...
					LEFT JOIN (
						...
						AND c.type = '{$pr}'
						GROUP BY pr_co_company_id
					) AS p_counter ON p_counter.pr_co_company_id = cbc.id";
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS 
				...
				{$lang_select}
				FROM ac_nr_pr_co_company cbc 
				INNER JOIN ac_nr_pr_co_company_data cd
				ON cd.pr_co_company_id = cbc.id
		...
		$sql = "SELECT n.*,
				...
				LEFT JOIN nr_user u 
				ON n.user_id = u.id
				LEFT JOIN ac_nr_pr_co_company c
				ON c.company_id = n.company_id
				LEFT JOIN ac_nr_pr_co_company_data cd
				...
				LEFT JOIN (
					...
					AND c.type = '{$pr}'
					GROUP BY pr_co_company_id
				) AS p_counter ON p_counter.pr_co_company_id = c.id
				...
				LEFT JOIN (
					...
					AND language <> 'en'
					GROUP BY pr_co_company_id
				) AS e_counter ON e_counter.pr_co_company_id = c.id
				...
				LEFT JOIN (
					...
					WHERE language IS NOT NULL
					GROUP BY pr_co_company_id
				) AS l_counter ON l_counter.pr_co_company_id = c.id
	...
	protected function render_auto_built_nr_list($chunkination, $results)
	{
		...
		$this->load->view('admin/nr_builder/menu');
		$this->load->view('admin/pre-content');
		$this->load->view("admin/nr_builder/pr_co/auto_built_newsrooms");
		$this->load->view('admin/post-content');
		$this->load->view('admin/footer');
	}
	...
	{
		$this->vd->heading = "Claimed Submissions (PR.Co)";
	...
	{
		if (!$id)
			$this->redirect('admin/nr_builder/pr_co');
		...
		if ($this->input->post('save'))
		{
			...
			$this->redirect("admin/nr_builder/pr_co/edit/{$id}");
		}
		...
		if ($modal == 1)
			$this->load->view('admin/nr_builder/pr_co/edit_with_modal');
		else
		{
			$this->load->view('admin/header');
			$this->load->view('admin/nr_builder/menu');
			$this->load->view('admin/pre-content');
			$this->load->view('admin/nr_builder/pr_co/edit');
			$this->load->view('admin/post-content');
			$this->load->view('admin/footer');
		}
	}
	...
	protected function fetch_results($chunkination, $filter = null)
	{
		...
		{
			$cat = Model_PR_Co_Category::find($filter_category);
			$list_filter = new stdClass();
			...
			$filter = "{$filter} AND c.pr_co_category_id = {$filter_category}";
		}
		...
		if ($filter_search = $this->input->get('filter_search'))
		{
			...
			else if ($filter_search == "READY_TO_BUILD_NEWSROOMS_NON_EN_ALL")
			{
				$this->vd->search_ready_newsrooms = 1;				
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS 
				...
				HAVING cd.pr_co_company_id IS NOT NULL
				{$lang_having}
				ORDER BY c.id DESC
				{$limit_str}";
		...
		$sql = "SELECT c.name, cd.*, 
				...
				e_counter.num_non_en_prs,
				l_counter.num_lang_checked,
				cd.pr_co_company_id AS source_company_id
				FROM ac_nr_pr_co_company c
				LEFT JOIN ac_nr_pr_co_company_data cd 
				ON cd.pr_co_company_id = c.id 
		...
		foreach ($results as $result)
		{
			...
			if ($result->soc_fb_feed_status == Model_PR_Co_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			if ($result->soc_twitter_feed_status == Model_PR_Co_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			if ($result->soc_gplus_feed_status == Model_PR_Co_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			if ($result->soc_youtube_feed_status == Model_PR_Co_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			if ($result->soc_pinterest_feed_status == Model_PR_Co_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;
		...
		$prs_modal = new Modal();
		$prs_modal->set_title("Company's PR.Co PRs");
	...
	public function contacts($pr_co_company_id)
	{
		$sql = "SELECT c.*, mc.area_of_specialization,
				is_press_contact, image_url
				FROM ac_nr_pr_co_contact mc
				INNER JOIN nr_company_contact c
				ON mc.company_contact_id = c.id
				WHERE mc.pr_co_company_id = {$pr_co_company_id}
				ORDER BY mc.is_press_contact DESC,
				c.id";

		$query = $this->db->query($sql);
		$results = Model_PR_Co_Contact::from_db_all($query);
		$this->vd->results = $results;

		$m_mw_comp = Model_PR_Co_Company::find($pr_co_company_id);
		$this->vd->company_name = $m_mw_comp->name;
		$this->load->view('admin/nr_builder/pr_co/contacts');

	}

	public function pr_links($pr_co_company_id)
	{
		$sql = "SELECT c.title, p.url, c.id
				FROM nr_content c
				INNER JOIN nr_pb_pr_co_content p
				ON p.content_id = c.id
				WHERE p.pr_co_company_id = {$pr_co_company_id}";

		$query = $this->db->query($sql);
		$results = Model_PB_PR_Co_Content::from_db_all($query);
		$this->vd->results = $results;

		$m_mw_comp = Model_PR_Co_Company::find($pr_co_company_id);
		$this->vd->company_name = $m_mw_comp->name;
		$this->load->view('admin/nr_builder/pr_co/pr_links');
	...
	protected function render_list($chunkination, $results)
	{
		...
		$this->vd->cb_cat = Model_PR_Co_Category::find_all();
		...
		$this->load->view('admin/nr_builder/menu');
		$this->load->view('admin/pre-content');
		$this->load->view('admin/nr_builder/pr_co/main');
		$this->load->view('admin/post-content');
		$this->load->view('admin/footer');
	}
	...
	public function retry_logo()
	{
		...
		if (! $comp = Model_PR_Co_Company::find("company_id", $id))
			return false;
	...
	{
		$pr_co_company_id = $this->input->post('pr_co_company_id');
		$is_logo_valid = $this->input->post('is_logo_valid');

		if (!$c_data = Model_PR_Co_Company_Data::find($pr_co_company_id))
			$this->json(0);
	...
	public function update_web_status()
	{
		...
		if (!$pr_co_comp = Model_PR_Co_Company_Data::find($pr_co_company_id))
			$this->json(0);
		
		else
		{
			if ($is_website_valid)
				$pr_co_comp->is_website_valid = 1;
			else
			{
				$pr_co_comp->website = null;
				$pr_co_comp->is_website_valid = 0;
				$pr_co_comp->website_source = Model_PR_Co_Company_Data::WEBSITE_SOURCE_NONE;
				$pr_co_comp->name = null;
			}
	...
	public function generate_tokens_for_all_generated_nrs()
	{
		...
		$query = $this->db->query($sql);
		$results = Model_PR_Co_Company::from_db_all($query);
		foreach ($results as $result)
		{
			$token = new Model_PR_Co_NR_Claim_Token();

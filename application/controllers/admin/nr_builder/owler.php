...
class Owler_Controller extends NR_Builder {

	public $title = 'Newsroom Builder | Owler';

	public function __construct()
	{
		parent::__construct();
		$this->vd->nr_source = Model_Company::SOURCE_OWLER;
		$this->nr_source = Model_Company::SOURCE_OWLER;
		$this->vd->nr_source_title = 'Owler';
	}

	public function index($chunk = 1)
	{	
		$this->redirect("admin/nr_builder/owler/all");
	}
	...
	public function instant_edit_save()
	{
		...
		{
			$c_data = Model_Owler_Company_Data::find($company_id);
				...
				|| $field_name == "soc_gplus" || $field_name == "soc_youtube" || $field_name == "soc_pinterest"))
		{
			...
			$c_data->{$feed_status_field} = Model_Owler_Company_Data::SOCIAL_NOT_CHECKED;
			$c_data->save();
			$response = "Saved successfully";
		}
	...
	protected function fetch_auto_built_nr_results($chunkination, $filter = null, $pre_exist_nr_websites_sql = '')
	{
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS 
				...
				FROM ac_nr_owler_company cbc 
				INNER JOIN ac_nr_owler_company_data cd
				ON cd.owler_company_id = cbc.id
		...
		$sql = "SELECT n.*,
				...
				LEFT JOIN nr_user u 
				ON n.user_id = u.id
				LEFT JOIN ac_nr_owler_company c
				ON c.company_id = n.company_id
				LEFT JOIN ac_nr_owler_company_data cd
	...
	{
		$this->vd->heading = "Claimed Submissions (Owler)";
	...
	{
		if (!$id)
			$this->redirect('admin/nr_builder/owler');
		...
		if ($this->input->post('save'))
		{
			...
			// $this->set_redirect("admin/nr_builder/owler/edit/{$id}");
			$this->set_redirect("admin/nr_builder/owler/all");
		}
		...
		if ($modal == 1)
			$this->load->view('admin/nr_builder/owler/edit_with_modal');
		else
		{
			$this->load->view('admin/header');
			$this->load->view('admin/nr_builder/menu');
			$this->load->view('admin/pre-content');
			$this->load->view('admin/nr_builder/owler/edit');
	...
	public function edit_save($id)
	{
		...
		if ($c_data->soc_fb)
		{
			if (Social_Facebook_Feed::is_valid($c_data->soc_fb))
				$c_data->soc_fb_feed_status = Model_Owler_Company_Data::SOCIAL_VALID;
			else
			{
				...
				$c_data->soc_fb = null;
				$c_data->soc_fb_feed_status = Model_Owler_Company_Data::SOCIAL_INVALID;
			}
		}
		...
		if ($c_data->soc_twitter)
		{
			if (Social_Twitter_Feed::is_valid($c_data->soc_twitter))
				$c_data->soc_twitter_feed_status = Model_Owler_Company_Data::SOCIAL_VALID;
			else
			{
				$warning_msg = "{$warning_msg} <br> Unable to validate the following social media ";
				...
				$c_data->soc_twitter_feed_status = Model_Owler_Company_Data::SOCIAL_INVALID;
			}
		}
		...
		if ($c_data->soc_gplus)
		{
			if (Social_GPlus_Feeds::is_valid($c_data->soc_gplus))
				$c_data->soc_gplus_feed_status = Model_Owler_Company_Data::SOCIAL_VALID;
			else
			{
				$warning_msg = "{$warning_msg} <br> Unable to validate the following social media ";
				...
				$c_data->soc_gplus_feed_status = Model_Owler_Company_Data::SOCIAL_INVALID;
			}
		}
		...
		if ($c_data->soc_youtube)
		{
			if (Social_Youtube_Feed::is_valid($c_data->soc_youtube))
				$c_data->soc_youtube_feed_status = Model_Owler_Company_Data::SOCIAL_VALID;
			else
			{
				$warning_msg = "{$warning_msg} <br> Unable to validate the following social media ";
				...
				$c_data->soc_youtube_feed_status = Model_Owler_Company_Data::SOCIAL_INVALID;
			}
		}
		...
		if ($c_data->soc_pinterest)
		{
			if (Social_Pinterest_Feed::is_valid($c_data->soc_pinterest))
				$c_data->soc_pinterest_feed_status = Model_Owler_Company_Data::SOCIAL_VALID;
			else
			{
				$warning_msg = "{$warning_msg} <br> Unable to validate the following social media ";
				...
				$c_data->soc_pinterest_feed_status = Model_Owler_Company_Data::SOCIAL_INVALID;
			}
		}
	...
	{		
		...
		{
			$cat = Model_Owler_Category::find($filter_category);
			...
			$filter = "{$filter} AND c.owler_category_id = {$filter_category}";
		}
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS 
				...
				HAVING cd.owler_company_id IS NOT NULL
				ORDER BY c.id DESC
				{$limit_str}";
		...
		$sql = "SELECT c.name, cd.*, 
				...
				p_counter.num_prs,
				cd.owler_company_id AS source_company_id
				FROM ac_nr_owler_company c
				LEFT JOIN ac_nr_owler_company_data cd 
				ON cd.owler_company_id = c.id 
				...
				LEFT JOIN (
					SELECT owler_company_id,
					COUNT(content_id) AS num_prs
					FROM nr_pb_owler_news 
					GROUP BY owler_company_id
				) AS p_counter ON p_counter.owler_company_id = c.id
		...
		foreach ($results as $result)
		{
			...
			if ($result->soc_fb_feed_status == Model_Owler_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			if ($result->soc_twitter_feed_status == Model_Owler_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			if ($result->soc_gplus_feed_status == Model_Owler_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			if ($result->soc_youtube_feed_status == Model_Owler_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			if ($result->soc_pinterest_feed_status == Model_Owler_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;
	...
	protected function render_list($chunkination, $results)
	{
		...
		$this->vd->cb_cat = Model_Owler_Category::find_all(null, array('name', 'asc'));
		...
		$prs_modal = new Modal();
		$prs_modal->set_title("Company's Owler PRs");
		...
		$this->load->view('admin/nr_builder/menu');
		$this->load->view('admin/pre-content');
		$this->load->view('admin/nr_builder/owler/main');
		$this->load->view('admin/post-content');
		$this->load->view('admin/footer');
	}

	public function pr_links($owler_company_id)
	{
		$sql = "SELECT c.title, p.url, p.actual_news_url
				FROM nr_content c
				INNER JOIN nr_pb_owler_news p
				ON p.content_id = c.id
				WHERE p.owler_company_id = {$owler_company_id}";

		$query = $this->db->query($sql);
		$results = Model_PB_Owler_News::from_db_all($query);
		
		foreach ($results as $result)
			$result->actual_news_url = urldecode($result->actual_news_url);

		$this->vd->results = $results;

		$m_mw_comp = Model_Owler_Company::find($owler_company_id);
		$this->vd->company_name = $m_mw_comp->name;
		$this->load->view('admin/nr_builder/owler/pr_links');
	...
	public function retry_logo()
	{
		...
		if (! $comp = Model_Owler_Company::find("company_id", $id))
			return false;

		$c_data = Model_Owler_Company_Data::find($comp->id);
	...
	{
		$owler_company_id = $this->input->post('owler_company_id');
		$is_logo_valid = $this->input->post('is_logo_valid');

		if (!$c_data = Model_Owler_Company_Data::find($owler_company_id))
			$this->json(0);
	...
	public function generate_tokens_for_all_generated_nrs()
	{
		...
		$query = $this->db->query($sql);
		$results = Model_Owler_Company::from_db_all($query);
		foreach ($results as $result)
		{
			$token = new Model_Owler_NR_Claim_Token();

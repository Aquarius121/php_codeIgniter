...
class CrunchBase_Controller extends NR_Builder {

	public $title = 'Newsroom Builder | Crunch Base';

	public function __construct()
	{
		parent::__construct();
		$this->vd->nr_source = Model_Company::SOURCE_CRUNCHBASE;
		$this->nr_source = Model_Company::SOURCE_CRUNCHBASE;
		$this->vd->nr_source_title = 'CrunchBase';
	}	
	...
	public function instant_edit_save()
	{
		...
		{
			$c_data = Model_CB_Company_Data::find($company_id);
				...
				|| $field_name == "soc_gplus" || $field_name == "soc_youtube" || $field_name == "soc_pinterest"))
		{
			...
			$c_data->{$feed_status_field} = Model_CB_Company_Data::SOCIAL_NOT_CHECKED;
			$c_data->save();
			$response = "Saved successfully";
		}
	...
	protected function fetch_auto_built_nr_results($chunkination, $filter = null, $pre_exist_nr_websites_sql = '')
	{
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS 
				...
				FROM ac_nr_cb_company cbc 
				INNER JOIN ac_nr_cb_company_data cd
				ON cd.company_id = cbc.id
		...
		$sql = "SELECT n.*,
				...
				LEFT JOIN nr_user u 
				ON n.user_id = u.id
				LEFT JOIN ac_nr_cb_company c
				ON c.company_id = n.company_id
				LEFT JOIN ac_nr_cb_company_data cd
	...
	{
		$this->vd->heading = "Claimed Submissions (CB)";
	...
	{
		if (!$id)
			$this->redirect('admin/nr_builder/crunchbase');
		...
		if ($this->input->post('save'))
		{
			...
			$this->redirect("admin/nr_builder/crunchbase/edit/{$id}");
		}
		...
		if ($modal == 1)
			$this->load->view('admin/nr_builder/crunchbase/edit_with_modal');
		else
		{
			$this->load->view('admin/header');
			$this->load->view('admin/nr_builder/menu');
			$this->load->view('admin/pre-content');
			$this->load->view('admin/nr_builder/crunchbase/edit');
	...
	public function edit_save($id)
	{
		...
		if ($c_data->soc_fb)
		{
			if (Social_Facebook_Feed::is_valid($c_data->soc_fb))
				$c_data->soc_fb_feed_status = Model_CB_Company_Data::SOCIAL_VALID;
			else
			{
				...
				$c_data->soc_fb = null;
				$c_data->soc_fb_feed_status = Model_CB_Company_Data::SOCIAL_INVALID;
			}
		}
		...
		if ($c_data->soc_twitter)
		{
			if (Social_Twitter_Feed::is_valid($c_data->soc_twitter))
				$c_data->soc_twitter_feed_status = Model_CB_Company_Data::SOCIAL_VALID;
			else
			{
				$warning_msg = "{$warning_msg} <br> Unable to validate the following social media ";
				...
				$c_data->soc_twitter_feed_status = Model_CB_Company_Data::SOCIAL_INVALID;
			}
		}
		...
		if ($c_data->soc_gplus)
		{
			if (Social_GPlus_Feeds::is_valid($c_data->soc_gplus))
				$c_data->soc_gplus_feed_status = Model_CB_Company_Data::SOCIAL_VALID;
			else
			{
				$warning_msg = "{$warning_msg} <br> Unable to validate the following social media ";
				...
				$c_data->soc_gplus_feed_status = Model_CB_Company_Data::SOCIAL_INVALID;
			}
		}
		...
		if ($c_data->soc_youtube)
		{
			if (Social_Youtube_Feed::is_valid($c_data->soc_youtube))
				$c_data->soc_youtube_feed_status = Model_CB_Company_Data::SOCIAL_VALID;
			else
			{
				$warning_msg = "{$warning_msg} <br> Unable to validate the following social media ";
				...
				$c_data->soc_youtube_feed_status = Model_CB_Company_Data::SOCIAL_INVALID;
			}
		}
		...
		if ($c_data->soc_pinterest)
		{
			if (Social_Pinterest_Feed::is_valid($c_data->soc_pinterest))
				$c_data->soc_pinterest_feed_status = Model_CB_Company_Data::SOCIAL_VALID;
			else
			{
				$warning_msg = "{$warning_msg} <br> Unable to validate the following social media ";
				...
				$c_data->soc_pinterest_feed_status = Model_CB_Company_Data::SOCIAL_INVALID;
			}
		}
	...
	{		
		...
		{
			$cat = Model_CB_Category::find($filter_category);
			...
			$filter = "{$filter} AND c.cb_category_id = {$filter_category}";
		}
		...
		if ($filter_search = $this->input->get('filter_search'))
		{
			...
			if ($filter_search == "READY_TO_BUILD_NEWSROOMS")	
			{
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS 
				...
				HAVING cd.company_id IS NOT NULL
				ORDER BY c.id DESC
				{$limit_str}";
		...
		$sql = "SELECT c.name, cd.*, 
				...
				cd.company_id AS source_company_id
				FROM ac_nr_cb_company c
				LEFT JOIN ac_nr_cb_company_data cd 
				ON cd.company_id = c.id 
		...
		foreach ($results as $result)
		{
			...
			if ($result->soc_fb_feed_status == Model_CB_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			if ($result->soc_twitter_feed_status == Model_CB_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			if ($result->soc_gplus_feed_status == Model_CB_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			if ($result->soc_youtube_feed_status == Model_CB_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			if ($result->soc_pinterest_feed_status == Model_CB_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			$company_name = trim($result->name);
			if (!empty($company_name) && $result->website && $result->email && $soc_count >= 1 && $soc_valid_count >= 1)
	...
	protected function render_list($chunkination, $results)
	{
		...
		$this->vd->cb_cat = Model_CB_Category::find_all();
		...
		$this->load->view('admin/nr_builder/menu');
		$this->load->view('admin/pre-content');
		$this->load->view('admin/nr_builder/crunchbase/main');
		$this->load->view('admin/post-content');
		$this->load->view('admin/footer');
	}
	...
	public function generate_tokens_for_all_generated_nrs()
	{
		...
		$query = $this->db->query($sql);
		$results = Model_CB_Company::from_db_all($query);
		foreach ($results as $result)
		{

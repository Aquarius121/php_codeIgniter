...
class PRWeb_Controller extends NR_Builder {
	
	public $title = 'Newsroom Builder | PRWeb';

	public function __construct()
	{
		parent::__construct();
		$this->vd->nr_source = Model_Company::SOURCE_PRWEB;
		$this->nr_source = Model_Company::SOURCE_PRWEB;
		$this->vd->nr_source_title = 'PRWeb';
		$this->vd->check_prn_sop_valid_lead = 1;
	}
	
	public function index($chunk = 1)
	{	
		$this->redirect("admin/nr_builder/prweb/all");
	}
	...
	public function instant_edit_save()
	{
		...
		{
			$c_data = Model_PRWeb_Company_Data::find($company_id);
				...
				|| $field_name == "soc_gplus" || $field_name == "soc_youtube" || $field_name == "soc_pinterest"))
		{
			...
			$c_data->{$feed_status_field} = Model_PRWeb_Company_Data::SOCIAL_NOT_CHECKED;
			$c_data->save();
			$response = "Saved successfully";
		}
	...
	protected function fetch_auto_built_nr_results($chunkination, $filter = null, $pre_exist_nr_websites_sql = '')
	{
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS 
				...
				FROM ac_nr_prweb_company cbc 
				INNER JOIN ac_nr_prweb_company_data cd
				ON cd.prweb_company_id = cbc.id
		...
		$query = $this->db->query($sql, array(Model_PRN_Valid_Company::SOURCE_PRWEB));
		$id_list = array();
		...
		$sql = "SELECT n.*,
				...
				LEFT JOIN nr_user u 
				ON n.user_id = u.id
				LEFT JOIN ac_nr_prweb_company c
				ON c.company_id = n.company_id
				LEFT JOIN ac_nr_prweb_company_data cd
	...
	{
		$this->vd->heading = "Claimed Submissions (PRWeb)";
	...
	{
		if (!$id)
			$this->redirect('admin/nr_builder/prweb');
		...
		if ($this->input->post('save'))
		{
			...
			$this->redirect("admin/nr_builder/prweb/edit/{$id}");
		}
		...
		if ($modal == 1)
			$this->load->view('admin/nr_builder/prweb/edit_with_modal');
		else
		{
			$this->load->view('admin/header');
			$this->load->view('admin/nr_builder/menu');
			$this->load->view('admin/pre-content');
			$this->load->view('admin/nr_builder/prweb/edit');
	...
	public function edit_save($id)
	{
		...
		if ($c_data->soc_fb)
		{
			if (Social_Facebook_Feed::is_valid($c_data->soc_fb))
				$c_data->soc_fb_feed_status = Model_PRWeb_Company_Data::SOCIAL_VALID;
			else
			{
				...
				$c_data->soc_fb = null;
				$c_data->soc_fb_feed_status = Model_PRWeb_Company_Data::SOCIAL_INVALID;
			}
		}
		...
		if ($c_data->soc_twitter)
		{
			if (Social_Twitter_Feed::is_valid($c_data->soc_twitter))
				$c_data->soc_twitter_feed_status = Model_PRWeb_Company_Data::SOCIAL_VALID;
			else
			{
				$warning_msg = "{$warning_msg} <br> Unable to validate the following social media ";
				...
				$c_data->soc_twitter_feed_status = Model_PRWeb_Company_Data::SOCIAL_INVALID;
			}
		}
		...
		if ($c_data->soc_gplus)
		{
			if (Social_GPlus_Feeds::is_valid($c_data->soc_gplus))
				$c_data->soc_gplus_feed_status = Model_PRWeb_Company_Data::SOCIAL_VALID;
			else
			{
				$warning_msg = "{$warning_msg} <br> Unable to validate the following social media ";
				...
				$c_data->soc_gplus_feed_status = Model_PRWeb_Company_Data::SOCIAL_INVALID;
			}
		}
		...
		if ($c_data->soc_youtube)
		{
			if (Social_Youtube_Feed::is_valid($c_data->soc_youtube))
				$c_data->soc_youtube_feed_status = Model_PRWeb_Company_Data::SOCIAL_VALID;
			else
			{
				$warning_msg = "{$warning_msg} <br> Unable to validate the following social media ";
				...
				$c_data->soc_youtube_feed_status = Model_PRWeb_Company_Data::SOCIAL_INVALID;
			}
		}
		...
		if ($c_data->soc_pinterest)
		{
			if (Social_Pinterest_Feed::is_valid($c_data->soc_pinterest))
				$c_data->soc_pinterest_feed_status = Model_PRWeb_Company_Data::SOCIAL_VALID;
			else
			{
				$warning_msg = "{$warning_msg} <br> Unable to validate the following social media ";
				...
				$c_data->soc_pinterest_feed_status = Model_PRWeb_Company_Data::SOCIAL_INVALID;
			}
		}
	...
	{		
		...
		{
			$cat = Model_PRWeb_Category::find($filter_category);
			...
			$filter = "{$filter} AND c.prweb_category_id = {$filter_category}";
		}
		...
		if ($filter_search = $this->input->get('filter_search'))
		{
			...
			$condition_name_invalid = "c.is_name_valid = 0";
			$condition_name_valid = "c.is_name_valid = 1";
			...
			elseif ($filter_search == "CHECK_NAME")	
			{
				$filter = "{$filter} AND {$condition_name}";
				$filter = "{$filter} AND {$condition_name_invalid}";
			}
			...
			else if ($filter_search == "READY_TO_BUILD_NEWSROOMS_NEW")	
			{
				...
				$filter = "{$filter} AND {$condition_social}";
				$filter = "{$filter} AND {$condition_name}";
				$filter = "{$filter} AND {$condition_name_valid}";

				$filter = "{$filter} AND c.id IN (
								SELECT prweb_company_id 
								FROM nr_content c
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS 
				...
				HAVING cd.prweb_company_id IS NOT NULL
				ORDER BY c.id DESC
				{$limit_str}";
				
		$query = $this->db->query($sql, Model_PRN_Valid_Company::SOURCE_PRWEB);
		$id_list = array();
		foreach ($query->result() as $row)
			$id_list[] = (int) $row->id;
		...
		$now = Date::$now->format(DATE::FORMAT_MYSQL);
		$sql = "SELECT c.name, c.is_name_valid,
				cd.*, u.email AS inews_user_email,
				p_counter.num_prs,
				cd.prweb_company_id AS source_company_id,
				pvc.is_prn_valid_lead,
				pvc.date_till_lead_valid
				FROM ac_nr_prweb_company c
				LEFT JOIN ac_nr_prweb_company_data cd 
				...
				LEFT JOIN (
					...
					FROM nr_pb_prweb_pr 
					GROUP BY prweb_company_id
				) AS p_counter ON p_counter.prweb_company_id = c.id
		...
		foreach ($results as $result)
		{
			...
			if ($result->soc_fb_feed_status == Model_PRWeb_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			if ($result->soc_twitter_feed_status == Model_PRWeb_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			if ($result->soc_gplus_feed_status == Model_PRWeb_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			if ($result->soc_youtube_feed_status == Model_PRWeb_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			if ($result->soc_pinterest_feed_status == Model_PRWeb_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			$company_name = trim($result->name);
			
			if ($result->website && $result->email && $soc_count >= 1 && $soc_valid_count >= 1 
				&& !empty($company_name) && $result->is_name_valid)	
		...
		$prs_modal = new Modal();
		$prs_modal->set_title("Company's PRWeb PRs");
	...
	public function pr_links($prweb_company_id)
	{
		$sql = "SELECT c.title, p.url
				FROM nr_content c
				INNER JOIN nr_pb_prweb_pr p
				ON p.content_id = c.id
				WHERE p.prweb_company_id = {$prweb_company_id}";

		$query = $this->db->query($sql);
		$results = Model_PB_PRWeb_PR::from_db_all($query);
		$this->vd->results = $results;

		$m_mw_comp = Model_PRWeb_Company::find($prweb_company_id);
		$this->vd->company_name = $m_mw_comp->name;
		$this->load->view('admin/nr_builder/prweb/pr_links');
	...
	protected function render_list($chunkination, $results)
	{
		...
		$this->vd->cb_cat = Model_PRWeb_Category::find_all();
		...
		$this->load->view('admin/nr_builder/menu');
		$this->load->view('admin/pre-content');
		$this->load->view('admin/nr_builder/prweb/main');
		$this->load->view('admin/post-content');
		$this->load->view('admin/footer');
	}
	...
	{
		$prweb_company_id = $this->input->post('prweb_company_id');
		$is_logo_valid = $this->input->post('is_logo_valid');

		if (!$c_data = Model_PRWeb_Company_Data::find($prweb_company_id))
			$this->json(0);
	...
	public function update_name_status()
	{
		$prweb_company_id = $this->input->post('prweb_company_id');
		$is_name_valid = $this->input->post('is_name_valid');

		if (!$prweb_comp = Model_PRWeb_Company::find($prweb_company_id))
			$this->json(0);
		
		else
		{
			if ($is_name_valid)
				$prweb_comp->is_name_valid = 1;
			else
				$prweb_comp->name = null;
	...
	public function generate_tokens_for_all_generated_nrs()
	{
		...
		$query = $this->db->query($sql);
		$results = Model_PRWeb_Company::from_db_all($query);
		foreach ($results as $result)
		{
			$token = new Model_PRWeb_NR_Claim_Token();

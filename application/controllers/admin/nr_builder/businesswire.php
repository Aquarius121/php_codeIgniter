...
class BusinessWire_Controller extends NR_Builder {

	public $title = 'Newsroom Builder | BusinessWire';

	public function __construct()
	{
		parent::__construct();
		$this->vd->nr_source = Model_Company::SOURCE_BUSINESSWIRE;
		$this->nr_source = Model_Company::SOURCE_BUSINESSWIRE;
		$this->vd->nr_source_title = 'BusinessWire';
	}

	public function index($chunk = 1)
	{	
		$this->redirect("admin/nr_builder/businesswire/all");
	}
	...
	public function instant_edit_save()
	{
		...
		{
			$c_data = Model_BusinessWire_Company_Data::find($company_id);
				...
				|| $field_name == "soc_gplus" || $field_name == "soc_youtube" || $field_name == "soc_pinterest"))
		{
			...
			$c_data->{$feed_status_field} = Model_BusinessWire_Company_Data::SOCIAL_NOT_CHECKED;
			$c_data->save();
			$response = "Saved successfully";
		}
	...
	protected function fetch_auto_built_nr_results($chunkination, $filter = null, $pre_exist_nr_websites_sql = '')
	{
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS 
				...
				FROM ac_nr_businesswire_company cbc 
				INNER JOIN ac_nr_businesswire_company_data cd
				ON cd.businesswire_company_id = cbc.id
		...
		$sql = "SELECT n.*,
				...
				LEFT JOIN nr_user u 
				ON n.user_id = u.id
				LEFT JOIN ac_nr_businesswire_company c
				ON c.company_id = n.company_id
				LEFT JOIN ac_nr_businesswire_company_data cd
	...
	{
		$this->vd->heading = "Claimed Submissions (BW)";
	...
	{
		if (!$id)
			$this->redirect('admin/nr_builder/businesswire');
		...
		if ($this->input->post('save'))
		{
			...
			// $this->set_redirect("admin/nr_builder/businesswire/edit/{$id}");
			$this->set_redirect("admin/nr_builder/businesswire/all");
		}

		$comp = Model_BusinessWire_Company::find($id);
		$c_data = Model_BusinessWire_Company_Data::find($id);
		$this->vd->comp = $comp;
		...
		if ($modal == 1)
			$this->load->view('admin/nr_builder/businesswire/edit_with_modal');
		else
		{
			$this->load->view('admin/header');
			$this->load->view('admin/nr_builder/menu');
			$this->load->view('admin/pre-content');
			$this->load->view('admin/nr_builder/businesswire/edit');
	...
	public function edit_save($id)
	{
		...
		if ($c_data->soc_fb)
		{
			if (Social_Facebook_Feed::is_valid($c_data->soc_fb))
				$c_data->soc_fb_feed_status = Model_BusinessWire_Company_Data::SOCIAL_VALID;
			else
			{
				...
				$c_data->soc_fb = null;
				$c_data->soc_fb_feed_status = Model_BusinessWire_Company_Data::SOCIAL_INVALID;
			}
		}
		...
		if ($c_data->soc_twitter)
		{
			if (Social_Twitter_Feed::is_valid($c_data->soc_twitter))
				$c_data->soc_twitter_feed_status = Model_BusinessWire_Company_Data::SOCIAL_VALID;
			else
			{
				$warning_msg = "{$warning_msg} <br> Unable to validate the following social media ";
				...
				$c_data->soc_twitter_feed_status = Model_BusinessWire_Company_Data::SOCIAL_INVALID;
			}
		}
		...
		if ($c_data->soc_gplus)
		{
			if (Social_GPlus_Feeds::is_valid($c_data->soc_gplus))
				$c_data->soc_gplus_feed_status = Model_BusinessWire_Company_Data::SOCIAL_VALID;
			else
			{
				$warning_msg = "{$warning_msg} <br> Unable to validate the following social media ";
				...
				$c_data->soc_gplus_feed_status = Model_BusinessWire_Company_Data::SOCIAL_INVALID;
			}
		}
		...
		if ($c_data->soc_youtube)
		{
			if (Social_Youtube_Feed::is_valid($c_data->soc_youtube))
				$c_data->soc_youtube_feed_status = Model_BusinessWire_Company_Data::SOCIAL_VALID;
			else
			{
				$warning_msg = "{$warning_msg} <br> Unable to validate the following social media ";
				...
				$c_data->soc_youtube_feed_status = Model_BusinessWire_Company_Data::SOCIAL_INVALID;
			}
		}
		...
		if ($c_data->soc_pinterest)
		{
			if (Social_Pinterest_Feed::is_valid($c_data->soc_pinterest))
				$c_data->soc_pinterest_feed_status = Model_BusinessWire_Company_Data::SOCIAL_VALID;
			else
			{
				$warning_msg = "{$warning_msg} <br> Unable to validate the following social media ";
				...
				$c_data->soc_pinterest_feed_status = Model_BusinessWire_Company_Data::SOCIAL_INVALID;
			}
		}
	...
	{		
		...
		{
			$cat = Model_BusinessWire_Category::find($filter_category);
			...
			$filter = "{$filter} AND c.businesswire_category_id = {$filter_category}";
		}
		...
		if ($filter_search = $this->input->get('filter_search'))
		{
			...
			else if ($filter_search == "READY_TO_BUILD_NEWSROOMS_NEW")	
			{
				...
				$filter = "{$filter} AND c.id IN (
								SELECT businesswire_company_id 
								FROM nr_content c
			...
			else if ($filter_search == "MISSING_LOGO_N_SOCIALS_NEWSROOMS")
			{
				...
				$filter = "{$filter} AND NULLIF(cd.logo_image_path, '') IS NULL";
				$filter = "{$filter} {$condition_social_missing}";
			}
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS 
				cd.businesswire_company_id as id 
				...
				HAVING cd.businesswire_company_id IS NOT NULL
				ORDER BY c.id DESC
				{$limit_str}";
		...
		$sql = "SELECT c.name, cd.*, 
				...
				p_counter.num_prs,
				cd.businesswire_company_id AS source_company_id
				FROM ac_nr_businesswire_company c
				LEFT JOIN ac_nr_businesswire_company_data cd 
				ON cd.businesswire_company_id = c.id 
				...
				LEFT JOIN (
					...
					GROUP BY businesswire_company_id
				) AS p_counter ON p_counter.businesswire_company_id = c.id
		...
		foreach ($results as $result)
		{
			...
			if ($result->soc_fb_feed_status == Model_BusinessWire_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			if ($result->soc_twitter_feed_status == Model_BusinessWire_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			if ($result->soc_gplus_feed_status == Model_BusinessWire_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			if ($result->soc_youtube_feed_status == Model_BusinessWire_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			if ($result->soc_pinterest_feed_status == Model_BusinessWire_Company_Data::SOCIAL_VALID)
				$soc_valid_count++;

			$company_name = trim($result->name);
			if ($result->website && $result->email && $soc_count >= 1 && $soc_valid_count >= 1
				&& !empty($company_name))
	...
	protected function render_list($chunkination, $results)
	{
		...
		$this->vd->cb_cat = Model_BusinessWire_Category::find_all();
		...
		$prs_modal = new Modal();
		$prs_modal->set_title("Company's BusinessWire PRs");
		...
		$this->load->view('admin/nr_builder/menu');
		$this->load->view('admin/pre-content');
		$this->load->view('admin/nr_builder/businesswire/main');
		$this->load->view('admin/post-content');
		$this->load->view('admin/footer');
	}

	public function pr_links($businesswire_company_id)
	{
		$sql = "SELECT c.title, p.url
				FROM nr_content c
				INNER JOIN nr_pb_businesswire_pr p
				ON p.content_id = c.id
				WHERE p.businesswire_company_id = {$businesswire_company_id}";

		$query = $this->db->query($sql);
		$results = Model_PB_BusinessWire_PR::from_db_all($query);
		$this->vd->results = $results;

		$m_mw_comp = Model_BusinessWire_Company::find($businesswire_company_id);
		$this->vd->company_name = $m_mw_comp->name;
		$this->load->view('admin/nr_builder/businesswire/pr_links');
	...
	public function retry_logo()
	{
		...
		if (! $comp = Model_BusinessWire_Company::find("company_id", $id))
			return false;

		$c_data = Model_BusinessWire_Company_Data::find($comp->id);
	...
	{
		$businesswire_company_id = $this->input->post('businesswire_company_id');
		$is_logo_valid = $this->input->post('is_logo_valid');

		if (!$c_data = Model_BusinessWire_Company_Data::find($businesswire_company_id))
			$this->json(0);
	...
	public function generate_tokens_for_all_generated_nrs()
	{
		...
		$query = $this->db->query($sql);
		$results = Model_BusinessWire_Company::from_db_all($query);
		foreach ($results as $result)
		{
			$token = new Model_BusinessWire_NR_Claim_Token();

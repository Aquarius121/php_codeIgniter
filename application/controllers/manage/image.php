...
class Image_Controller extends Manage_Base {
	...
	public function upload()
	{
		...
		if ($si_original->size() > $this->conf('max_image_file_size'))
		{
		...
		$response['size'] = array();
		$response['size']['width'] = $im_original->width();
		$response['size']['height'] = $im_original->height();
		$response['files'] = array();
		$response['files']['original'] = $si_original->url();
		
		$variants = $this->input->post('variants');
		if (!$variants) $variants = array('thumb','finger','cover','cover-website');
	...
	public function remove()
	{
		...
		if (!$image->company_id) return $this->json(false);

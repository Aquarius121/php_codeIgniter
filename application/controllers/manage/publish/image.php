...
class Image_Controller extends Content_Base { 
	...
	protected $content_type = Model_Content::TYPE_IMAGE;	
	...
	protected function process_results($results)
	{
		...
		$sql = "SELECT * FROM nr_pb_image
			WHERE content_id IN ({$content_id_str})
			ORDER BY FIELD(content_id, {$content_id_str})";
	...
	public function edit_save()
	{
		...
		else
		{
			...
			if (!$m_pb_image) $m_pb_image = new Model_PB_Image();			

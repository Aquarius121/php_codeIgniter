...
class Contact_Controller extends Manage_Base { 
	...
	public function edit_save()
	{
		...
		{
			if ($post['is_main_contact'])
				Detached_Session::write('nr_contact', $contact);
			Detached_Session::write('m_company_contact', $contact);
			
			$preview_url = $contact->url();
			$preview_url = Detached_Session::save($this->newsroom, $preview_url);
	...
	public function search($chunk = 1)
	{
		...
		$terms_sql = sql_search_terms(array(
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS * 
			...
			ORDER BY cc.first_name ASC, cc.last_name ASC
			{$limit_str}";
	...
	public function delete($contact_id)
	{
		...
		if ($this->input->post('confirm'))
		{
			...
			$feedback = new Feedback('success', 'Deleted!', 'The contact has been removed.');
			$this->add_feedback($feedback);

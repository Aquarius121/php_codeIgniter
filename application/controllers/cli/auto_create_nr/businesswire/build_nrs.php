...
class Build_NRs_Controller extends Auto_Create_NR_Base { 
	...
	public function index($businesswire_company_ids_list = null)
	{
		set_time_limit(3000);

		$businesswire_company_ids = unserialize($businesswire_company_ids_list);

		if (empty($businesswire_company_ids) || !is_array($businesswire_company_ids) || !count($businesswire_company_ids))
			return false;

		$id_list = sql_in_list($businesswire_company_ids);

		$sql = "SELECT id
				FROM ac_nr_businesswire_company
				WHERE id IN ({$id_list})
		...
		while (1)
		{
			...
			$comp = Model_BusinessWire_Company::from_db($result);
			if (!$comp) break;
	...
	protected function build($id = null)
	{
		...
		$bw_cat = Model_BusinessWire_Category::find($comp->businesswire_category_id);
		...
		if ($newsroom->company_id)
		{
			$sql = "UPDATE nr_content
					...
					AND id IN (
						...
						WHERE businesswire_company_id = ?)";

			$this->db->query($sql, array($newsroom->company_id,	$c_data->businesswire_company_id));
		}
		...
		$company_profile->beat_id = value_or_null($bw_cat->newswire_beat_id);
		...
		$this->update_company_source($newsroom->company_id, Model_Company::SOURCE_BUSINESSWIRE);

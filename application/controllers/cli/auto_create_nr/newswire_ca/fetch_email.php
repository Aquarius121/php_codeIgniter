...
class Fetch_Email_Controller extends CLI_Base {
	...
	public function get($c_data)
	{
		...
		$m_f_email = new Model_Newswire_CA_Fetch_Email();
		$m_f_email->newswire_ca_company_id = $c_data->newswire_ca_company_id;
		...
		{			
			...
			$this->update_newswire_ca_c_data($c_data->newswire_ca_company_id, $email);
			return;
		}
	...
	protected function update_newswire_ca_c_data($newswire_ca_company_id, $email)
	{
		$c_data = Model_Newswire_CA_Company_Data::find($newswire_ca_company_id);
		$c_data->email = $email;

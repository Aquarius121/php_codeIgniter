...
class Crawl_Company_Website_Controller extends Auto_Create_NR_Base {
	...
	public function index()
	{
		...
		$sql = "SELECT cd.website, cd.newswire_ca_company_id
	...
	public function get($c_data)
	{
		...
		$web_crawled = new Model_Newswire_CA_Website_Crawled();
		$web_crawled->newswire_ca_company_id = $c_data->newswire_ca_company_id;
		...
		foreach($html->find('a') as $element)
		{
			...
			if (preg_match($pattern_fb, $href, $match)) 
			{
				...
				if (!empty($soc_fb))
				{
					if (empty($c_data->soc_fb) || $c_data->soc_fb_feed_status == Model_Newswire_CA_Company_Data::SOCIAL_INVALID)
					{
			...
			if (preg_match($pattern_twitter, $href, $match)) 
			{
				...
				{
					if (empty($c_data->soc_twitter) || $c_data->soc_twitter_feed_status == Model_Newswire_CA_Company_Data::SOCIAL_INVALID)
					{
			...
			if (preg_match($pattern_pinterest, $href, $match)) 
			{
				...
				{
					if (empty($c_data->soc_pinterest) || $c_data->soc_pinterest_feed_status == Model_Newswire_CA_Company_Data::SOCIAL_INVALID)
					{
			...
			if (preg_match($pattern_youtube, $href, $match)) 
			{
				...
				{
					if (empty($c_data->soc_youtube) || $c_data->soc_youtube_feed_status == Model_Newswire_CA_Company_Data::SOCIAL_INVALID)
					{

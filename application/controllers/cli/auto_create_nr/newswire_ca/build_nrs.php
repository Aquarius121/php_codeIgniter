...
class Build_NRs_Controller extends Auto_Create_NR_Base { 
	...
	public function index($newswire_ca_company_ids_list = null)
	{
		set_time_limit(3000);

		$newswire_ca_company_ids = unserialize($newswire_ca_company_ids_list);

		if (empty($newswire_ca_company_ids) || !is_array($newswire_ca_company_ids) || !count($newswire_ca_company_ids))
			return false;

		$id_list = sql_in_list($newswire_ca_company_ids);
		...
		while (1)
		{
			...
			$comp = Model_Newswire_CA_Company::from_db($result);
			if (!$comp) break;
	...
	protected function build($id = null)
	{
		...
		$newswire_ca_cat = Model_Newswire_CA_Category::find($comp->newswire_ca_category_id);
		...
		if ($newsroom->company_id)
		{
			$sql = "UPDATE nr_content
					...
					AND id IN (
						...
						WHERE newswire_ca_company_id = ?)";

			$this->db->query($sql, array($newsroom->company_id, $c_data->newswire_ca_company_id));
		...
		$company_profile->beat_id = value_or_null($newswire_ca_cat->newswire_beat_id);
		...
		$this->update_company_source($newsroom->company_id, Model_Company::SOURCE_NEWSWIRE_CA);

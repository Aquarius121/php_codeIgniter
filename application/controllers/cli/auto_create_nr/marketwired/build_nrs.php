...
class Build_NRs_Controller extends Auto_Create_NR_Base { 
	...
	public function index($marketwired_company_ids_list = null)
	{
		set_time_limit(3000);

		$marketwired_company_ids = unserialize($marketwired_company_ids_list);

		if (empty($marketwired_company_ids) || !is_array($marketwired_company_ids) || !count($marketwired_company_ids))
			return false;

		$id_list = sql_in_list($marketwired_company_ids);

		$sql = "SELECT id
				FROM ac_nr_marketwired_company
				WHERE id IN ({$id_list})
		...
		while (1)
		{
			...
			$comp = Model_MarketWired_Company::from_db($result);
			if (!$comp) break;
	...
	protected function build($id = null)
	{
		...
		$marketwired_cat = Model_MarketWired_Category::find($comp->marketwired_category_id);
		...
		if ($newsroom->company_id)
		{
			$sql = "UPDATE nr_content
					...
					AND id IN (
						...
						WHERE marketwired_company_id = ?)";

			$this->db->query($sql, array($newsroom->company_id,
							$c_data->marketwired_company_id));
		}
		...
		$company_profile->beat_id = value_or_null($marketwired_cat->newswire_beat_id);
		...
		$this->update_company_source($newsroom->company_id, Model_Company::SOURCE_MARKETWIRED);

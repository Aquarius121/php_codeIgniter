...
class Crawl_Company_Website_Controller extends Auto_Create_NR_Base {
	...
	public function get($c_data)
	{
		...
		$web_crawled = new Model_MarketWired_Website_Crawled();
		$web_crawled->marketwired_company_id = $c_data->marketwired_company_id;
		...
		$c_data = Model_MarketWired_Company_Data::find($c_data->marketwired_company_id);
		...
		foreach($html->find('a') as $element)
		{
			...
			if (preg_match($pattern_fb, $href, $match)) 
			{
				...
				if (!empty($soc_fb))
				{
					if (empty($c_data->soc_fb) || $c_data->soc_fb_feed_status == Model_MarketWired_Company_Data::SOCIAL_INVALID)
					{
						$c_data->soc_fb = $soc_fb;
						$c_data->soc_fb_feed_status = Model_MarketWired_Company_Data::SOCIAL_NOT_CHECKED;
					}
				}
			}
			...
			if (preg_match($pattern_twitter, $href, $match)) 
			{
				...
				{
					if (empty($c_data->soc_twitter) || $c_data->soc_twitter_feed_status == Model_MarketWired_Company_Data::SOCIAL_INVALID)
					{
						$c_data->soc_twitter = $soc_twitter;
						$c_data->soc_twitter_feed_status = Model_MarketWired_Company_Data::SOCIAL_NOT_CHECKED;
					}
				}
			}
			...
			if (preg_match($pattern_pinterest, $href, $match)) 
			{
				...
				{
					if (empty($c_data->soc_pinterest) || $c_data->soc_pinterest_feed_status == Model_MarketWired_Company_Data::SOCIAL_INVALID)
					{
						$c_data->soc_pinterest = $soc_pinterest;
						$c_data->soc_pinterest_feed_status = Model_MarketWired_Company_Data::SOCIAL_NOT_CHECKED;
					}
				}
			}
			...
			if (preg_match($pattern_youtube, $href, $match)) 
			{
				...
				{
					if (empty($c_data->soc_youtube) || $c_data->soc_youtube_feed_status == Model_MarketWired_Company_Data::SOCIAL_INVALID)
					{
						$c_data->soc_youtube = $soc_youtube;
						$c_data->soc_youtube_feed_status = Model_MarketWired_Company_Data::SOCIAL_NOT_CHECKED;
					}
				}
			}
			...
			if (preg_match($pattern_gplus, $href, $match)) 
			{
				...
				{					
					if (empty($c_data->soc_gplus))
					{
						...
						$c_data->soc_gplus_feed_status = Model_MarketWired_Company_Data::SOCIAL_NOT_CHECKED;
					}
				}
			}
		}

...
class Fetch_Email_Controller extends Auto_Create_NR_Base {
	...
	public function get($c_data)
	{
		...
		$m_f_email = new Model_Owler_Fetch_Email();
		$m_f_email->owler_company_id = $c_data->owler_company_id;
		...
		{	
			...
			$this->update_owler_c_data($c_data->owler_company_id, $email);
			return;
		}
	...
	public function update_owler_c_data($owler_company_id, $email)
	{
		$c_data = Model_Owler_Company_Data::find($owler_company_id);
		$c_data->email = $email;
		$c_data->save();
	}
}

...
abstract class Video {
	...
	const PROVIDER_VIMEO = 'Video_Vimeo';
	
	public static function providers()
	{
		return array(static::PROVIDER_YOUTUBE, static::PROVIDER_VIMEO);
	}
	...
	public static function get_instance($provider, $video_id = null)
	{
		if (static::is_valid_provider($provider))
		{
			...
			if ($video_id !== null)	$instance->video_id = 
	...
	public static function resolve($provider, $video_id, $download_thumb = false)
	{
		...
		if ($download_thumb)
		{
			...
			$si_cover_web = $si_original->from_this_resized($v_sizes['cover-website']);
			$si_original->move();
			...
			$image->add_variant($si_cover_web->save_to_db(), 'cover-website');
		}

...
class Newsroom_Assist {
	...
	{		
		// replace domain extensions
		$name = preg_replace('/\.com/', null, $name);
		$name = preg_replace('/\.org/', null, $name);
		$name = preg_replace('/\.net/', null, $name);
		$name = preg_replace('/\.info/', null, $name);
		
		// replace non-allowed characters
		$name = preg_replace('#[^a-z0-9]#is', null, $name);
	...
	{
		$ci =& get_instance();
		if ($ci->env['environment'] === 'development')
			return true;
	...
	{
		do { $random = md5(UUID::create()); }
		while (!Model_Newsroom::name_available($random));
		return $random;
	}	

...
jQuery.fn.deserialize = function( data, options ) {
    ...
    if ( jQuery.isArray( data ) ) {
        ...
        for ( key in data ) {
            var isArray = jQuery.isArray( value = data[ key ] );
            if (isArray) {
            	var isIndexed = isArrayIndexed(value);
            	push.apply( normalized, jQuery.map( value, function( v, sk ) {
                	if (isIndexed)
                       return { name: key + "[]", value: v };
                  else return { name: key + "[" + sk + "]", value: v };
                }));
            } else { 

...
function fetch_facebook_numeric_id(id)
{
	var output;      
	jQuery.ajax({            
...
(function($){
	...
	$.extend(SocialStreamObject.prototype, {
		...
		create: function(el, options) {
			...
			this.defaults = {
				feeds: {
					...
					twitter: {
						...
						url: 'assets/lib/social-stream/twitter.php',
						icon: 'twitter.png'
					},
					google: {
						id: '',
						...
						api_key: 'AIzaSyCcCf8wuTd2zUDbSUwY94eizzqL7TiLkTs',
						image_height: 75,
						image_width: 75,
						shares: true,
						icon: 'google.png'
			...
			this.o = {}, this.timer_on = 0, this.id = 'dcsns-'+$(el).index(), this.timerId = '', this.o = $.extend(true,this.defaults,options), opt = this.o, $load = $('<div class="dcsns-loading">loading...</div>');
	...
	function getFeed(type,id,path,o,obj,opt,f1,f2,intro,feed,fn){
		...
		switch (type) {
			...
			case 'facebook':
			var cp = id.split('/');
			// id = fetch_facebook_numeric_id(id);
			...
			success: function(a){
				...
				if(error == ''){
					$.each(a, function(i,item){
						if(i < n){
							// new condition added by Muhammad.
							if (type == 'rss')
							{
								var html = [], q = item.link, u='<a href="'+href+'">'+id+'</a>', w='', x = '<a href="'+q+'" class="rss_header_title">'+item.title+'</a>', y='', z='', zz='', m='', d = item.publishedDate, sq = q, st = item.title, s = '';
							}
							else if (type == 'facebook')
							{
								
								if (item.title.substr(0,7) == "http://")
									item.title="";
								var exp = /(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/i;
								item.title = item.title.replace(exp, "");
								
								var html = [], q = item.link, u='<a href="'+href+'">'+id+'</a>', w='', x = '<a href="'+q+'" class="rss_header_title">'+item.title+'</a>', y='', z='', zz='', m='', d = item.publishedDate, sq = q, st = item.title, s = '';
							}
							else
							{
								var html = [], q = item.link, u='<a href="'+href+'">'+id+'</a>', w='', x = '<a class="rss_header_title" href="'+q+'" >'+item.title+'</a>', y='', z='', zz='', m='', d = item.publishedDate, sq = q, st = item.title, s = '';
							}
							switch(type)
							{
								case 'facebook':
							...
							$.each(o.out.split(','), function(i,v){
								...
								switch(v)
								{
									...
									break;
									case 'title':
									/*** Code below is added by Muhammad *****/
									if (type == 'rss')
									{										
										var my_d = "";
										if (my_d1 = new Date(d))
										{
											my_date = my_d1.getDate();
											my_month = my_d1.getMonth();
											var monthNames = ["Jan", "Feb", "Mar", "Apr", "May", "Jun",
											"Jul", "Aug", "Sep", "Oct", "Nov", "Dec" ];
											my_mon = monthNames[my_month];
											my_year = my_d1.getFullYear();
											if (my_date)
											{												
												my_d =  my_mon + " " + my_date + ", " + my_year;
												my_d = " - <span class='ln-date'>" + my_d + "</span>";
											}
											//alert(my_d);
										}
										
										y = "<div id='rss_header'><span class='ln-category-type'>Blog Post</span>" + 
												my_d + "</div>";
										x = y + x;
									}
									/**** New code ends here ********/									
									...
									if(type == 'rss'){
										...
										if (src.substr(0,4) == 'http')		
											y = src ? '<a href="'+q+'" class="thumb"><img src="'+src+'" alt="" class="rss_thumb" /></a>' : '' ;
									}
									zz += y;
									break;
									case 'text':
									// new condition added by Muhammad
									/*if (type == 'rss')*/
										z = "<span class='rss_text'>" + z + "</span>";
										if (type == 'facebook')
										{	
											urlRegex = /href="(https?:\/\/[^\s]+)"/g;
											z = z.replace(urlRegex, 'href="'+q+'"');
										}
									zz += z;
									break;
									case 'user':
							...
							if (type == "rss")
								var out = '<li style="margin-left:9px !important; margin-top:12px !important" rel="'+ob+'" class="rss_box dcsns-li dcsns-'+type+' dcsns-feed-'+fn+'">'+w+'<div class="inner">'+zz+'<span class="clear"></span></div>'+'</li>', str = opt.remove;
							else
								var out = '<li style="margin-left:9px !important; margin-top:12px !important" rel="'+ob+'" class="dcsns-li dcsns-'+type+' dcsns-feed-'+fn+'">'+w+'<div class="inner">'+zz+'<span class="clear"></span></div>'+zintro+icon+'</li>', str = opt.remove;
							...
							if( str.indexOf( q ) !== -1 && q != '' ){
								...
								if(opt.max == 'days'){
									...
									if (inews_feed_added == 0)
									{
										$('.ln-block').each(function() {
											$(this).removeClass('ln-block');
											$(this).addClass('hidden');
											$('#ln-container').addClass('hidden');
											var inews_text = '<li class="dcsns-li dcsns-facebook dcsns-feed-0 isotope-item" rel="2" style="margin-left: 9px ! important; margin-top: 12px ! important; position: absolute; left: 0px; top: 0px; opacity: 1; transform: translate(239px, 2px) scale(1); padding-bottom:0px !important"><div class="ln-block-nr-main">';
											inews_text += $(this).html();
											inews_text += '</div></li>'
											list.push(inews_text);											
...
jQuery(window).load(function(){
	...
	jQuery('.dcsns-facebook .section-text a').each(function(i){
		...
		// New code by Muhammad
		var href = decodeURIComponent(txt.replace('/profile.php?id=', 'http://facebook.com/profile.php?id='));
		if (href.substr(0,1) == "/")
			href = "http://facebook.com"+href;
		href = href.replace("http://l.facebook.com/l.php?u=", "");
		href = href.replace("https://l.facebook.com/l.php?u=", "");
		href = href.replace("http://www.facebook.com/l.php?u=", "");
		href = href.replace("https://www.facebook.com/l.php?u=", "");
		href = href.split('&');
		// New code ends here		

...
(function() {
	...
	CKEDITOR.plugins.add('nanospell', {
		...
		init: function(editor) {
			...
			editor.ui.addButton('nanospell', {
				label: 'Enable/Disable Spell Check',
				command: 'nanospell',
				toolbar: 'nanospell',
	...
	function resolveAjaxHandler() {
		// we should use not use the assets server as it requires PHP and 
		// we must be able to POST and view JSON information. 
		return CKEDITOR_BASEPATH.replace(/^https?:\/\/[^\/]+/, "") 
			+ "nanospell/server/ajax/php/enchant.php";
	}

...
(function($){
		...
		create: function(el, options) {
			...
			this.defaults = {
				...
				twitter: {
					...
					url: 'assets/lib/social-media-tabs/twitter.php',
					icon: 'twitter.png'
				},
				facebook: {
					title: 'Facebook',
				...
				youtube: {
					...
					subscribe: true,
					api_key: 'AIzaSyCS4ZJDm06RqU3ZHdNZ-7wE_IJbmN9PqlY',
					icon: 'youtube.png'
				},
				flickr: {
					title: 'Flickr',
				...
				active: 'active',
				zopen: 1000,
				imagePath: 'assets/lib/social-media-tabs/images/icons/'
			};
			this.o = {};
			this.timer_on = 0;
			...
			wb += 14; // new line added by Muhammad
			hb -= 14; // new line added by Muhammad
			
			$s.css({height: hb+'px', width: wb+'px'});
			// $('.tab-inner',$s).css({height: hb+'px', width: wb+'px'}); // amended below
			$('.tab-inner',$s).css({height: hb+'px', width: wb+'px', margin: '-7px 0 14px -7px'});
	...
	function createWidget(obj,type,id,o,w,h,share,tweetId){
		...
		switch (type) {
			...
			url = cp.length > 1 ? url1 + 'url=timeline&screen_name='+cp[0]+'&per_page='+n+param : url1 + 'url=timeline&screen_name='+id+'&count='+n+param;
			...
			if(o.subscribe && is_channel == 0)
				c += '<iframe src="https://www.youtube.com/subscribe_widget?p='+id+'" class="youtube-subscribe" scrolling="no" frameBorder="0"></iframe>';
			
			c += stream;


			function load_channel_feeds(channel_id)
			{
				url = "https://www.googleapis.com/youtube/v3/search?part=snippet";
				url = url+"&channelId="+channel_id+"&order=date&key="+o.api_key+"&maxResults=20";
				getFeed(ti,type,url,data,o,id,share,tweetId);

				if (!is_channel)
					$('#dcsmt-0-youtube').css('margin-top', "-30px");
			}

			if (is_channel == 0)
			{
				// as per youtube api v3 we need to call the 
				// channels api for the  user to get channel
				// and then call the api to fetch videos

				api_url = "https://www.googleapis.com/youtube/v3/channels?part=contentDetails";
				api_url = api_url+"&forUsername="+id+"&key="+o.api_key;
				...
				load_channel_feeds(id);
			

			/*alert(channel_id);
			url = "https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=UCB8V3Xphfkmh8QKsXtsFiQg";
			url = url+"&order=date&key="+o.api_key+"&maxResults=20";
			getFeed(ti,type,url,data,o,id,share,tweetId);
			*/
					...
					$.each(a, function(i,item){
						if(i < o.limit){
							...
							switch(type)
							{
								...
								var link = "https://youtube.com/watch?v="+video_id;
								item.link = link;
								x = "<a href='"+item.link+"'>"+item.snippet.title+"</a><br>";
								html.push(x);
								var v = [];
								v = parseQ(item.link);
	...
	function getFrame(src,w,h){
		html = '<iframe src="'+src+'" scrolling="no" frameborder="0" style="border: none; background: #fff; overflow: hidden; width: '+w+'px; height: '+h+'px;" allowTransparency="true" class="aaaa"></iframe>';
		return html;
	};
	...
	function getLinkedIn(id,a,b,w){
		if(id.indexOf('in/') == 0)
		{
			id = "https://www.linkedin.com/"+id;
			out = '<script type="IN/MemberProfile" data-width="'+w+'" data-id="'+id+'" data-related="true" data-format="inline"></script>';
		}
		else
		{
